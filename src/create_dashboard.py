"""Generate the showcase dashboard and supporting figures.
...
def load_data() -> pd.DataFrame:
    """Load the sales workbook and add time-based analysis fields."""
...
def build_metrics(frame: pd.DataFrame) -> dict[str, object]:
    ...
    return {
        ...
        "top_pizzas": (
            frame.groupby("pizza_name")["total_price"]
            .sum()
            .nlargest(5)
            .sort_values()
        ),
        "weekday_orders": (
            frame.groupby("day_of_week")["order_id"]
            .nunique()
            .reindex(day_order)
        ),
...
def save_supporting_figures(metrics: dict[str, object]) -> None:
    ...
    colors = [COLORS["accent"] if day == "Friday" else COLORS["ink"] for day in weekday_orders.index]
    ...
    fig, ax = plt.subplots(figsize=(10, 5.5), facecolor=COLORS["white"])
    ax.plot(hourly_orders.index, hourly_orders.values, color=COLORS["accent"], linewidth=2.5, marker="o")
    style_axis(ax)
...
def save_dashboard(frame: pd.DataFrame, metrics: dict[str, object]) -> None:
    ...
    top_axis.set_title("Top products by revenue", loc="left", color=COLORS["ink"], weight="bold", pad=14)
    ...
    weekday_orders = metrics["weekday_orders"]
    weekday_colors = [
        COLORS["accent"] if day == "Friday" else COLORS["ink"]
        for day in weekday_orders.index
    ]
    ...
    weekday_axis.set_title("Demand builds toward Friday", loc="left", color=COLORS["ink"], weight="bold", pad=12)
    ...
    hourly_axis.set_title("Lunch and dinner drive order volume", loc="left", color=COLORS["ink"], weight="bold", pad=12)
...
def main() -> None:
    ...
    save_supporting_figures(metrics)
    save_dashboard(frame, metrics)
    print(f"Dashboard and figures written to {OUTPUT_DIR}")
